...
class MappingDataCollector(params: Map[String, String]) extends DataCollector(params) {
    ...
    override def collectActivityData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        var mappingPath = params(Constants.Parameters.MAPPING)
        
        var lstActivityFiles = DataUtils.getListFiles(activityPath, logDate, timing);
        var activityDF = sqlContext.read.parquet(lstActivityFiles: _*).select("game_code", calcId).distinct
        
        var lstMappingFiles = DataUtils.getListFiles(mappingPath, logDate, timing);
        var countryDF = sqlContext.read.parquet(lstMappingFiles: _*).sort("log_date").dropDuplicates(Seq(calcId)).select(calcId,"country_code")
    ...
    override def collectAccRegisterData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        var regDF = sqlContext.read.parquet(lstRegFiles: _*).select("game_code", calcId).distinct
    ...
    override def collectPaymentData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        var joinDF = paymentDF.as('p).join(countryDF.as('c), 
                ...
                .select("p.game_code", "p.log_date","p." + calcId, "p.net_amt", "c.country_code")
        //joinDF.show
    ...
    override def collectFirstChargeData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        var firstDF = sqlContext.read.parquet(lstFirstChargeFiles: _*)
        ...
        var joinDF = joinCountryDF.as('c).join(paymentDF.as('p), 
                ...
                .select("p.game_code", "p.log_date","c." + calcId, "p.net_amt", "c.country_code")
        //joinDF.show
    ...
    def collectNewUserRevenueData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        var joinDF = countryJoinDF.as('cp).join(regDF.as('r), 
                ...
                .select("cp.game_code", "cp.log_date","cp." + calcId, "cp.net_amt", "cp.country_code")
        //joinDF.show
    ...
    def collectNewUserRetentionData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        if (!DataUtils.isEmpty(regFile)) {
            var regDF = sqlContext.read.parquet(regFile).select(calcId)
            
            var activityFile = DataUtils.getFile(activityPath, logDate);
            var activityDF = sqlContext.read.parquet(activityFile).select(calcId).distinct
            
            var currentMappingFile = DataUtils.getFile(countryPath, logDate);
            var currentCountryDF = sqlContext.read.parquet(currentMappingFile).sort("log_date").dropDuplicates(Seq(calcId)).select(calcId,"country_code")
            
            var prevMappingFile = DataUtils.getFile(countryPath, logDate, timing);
            var prevCountryDF = sqlContext.read.parquet(prevMappingFile).sort("log_date").dropDuplicates(Seq(calcId)).select(calcId,"country_code")
    ...
    def collectUserRetentionData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        if (!DataUtils.isEmpty(prevActivityFile)) {
            ...
            var prevActivityDF = sqlContext.read.parquet(prevActivityFile).select(calcId).distinct
    ...
    def collectFirstChargeRetentionData(sqlContext: SQLContext, params: Map[String, String]): DataFrame = {
        ...
        if (!DataUtils.isEmpty(firstFile)) {
            ...
            var firstDF = sqlContext.read.parquet(firstFile).select(calcId)
            
            var payFile = DataUtils.getFile(payPath, logDate);
            var payDF = sqlContext.read.parquet(payFile).select(calcId).distinct
