...
object GroupFirstChargeReport extends BaseReport {
    ...
    def store(dataframe: DataFrame): Unit = {
        ...
        dataframe.collect().foreach { row =>
            ...
            output = KpiGroupFormat(source, gameCode, groupId, reportDate, createDate, DataUtils.getKpiId(calcId, Constants.Kpi.NEW_PAYING_NET_REVENUE, timing), totalNetRevenue) :: output
            output = KpiGroupFormat(source, gameCode, groupId, reportDate, createDate, DataUtils.getKpiId(calcId, Constants.Kpi.NEW_PAYING_GROSS_REVENUE, timing), totalGrossRevenue) :: output
            
            Common.logger("groupId: " + groupId + ", first charge: " + totalPaying + ", net revenue: " + totalNetRevenue + ", gross revenue: " + totalGrossRevenue)
        }
