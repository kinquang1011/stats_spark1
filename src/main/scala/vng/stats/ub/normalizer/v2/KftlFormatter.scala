...
object KftlFormatter extends IngameFormatter("kftl", "kfcv") {
    ...
    override def loginLogoutFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        def loginFilter(arr: Array[String]): Boolean = {
            return (arr.length == 13 && arr(6).startsWith(logDate))
        }
        ...
        def loginGenerate(arr: Array[String]): Row = {
            ...
            Row(gameCode, arr(6), "login", arr(2), arr(1), arr(4), "0")
        }
        
        def logoutGenerate(arr: Array[String]): Row = {

            Row(gameCode, arr(6), "logout", arr(2), arr(1), arr(5), arr(8))
        }

        var sf = Constants.LOGIN_LOGOUT_FIELD
        var loginMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.SID, sf.LEVEL, sf.ONLINE_TIME)
        var logoutMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.SID, sf.LEVEL, sf.ONLINE_TIME)
    ...
    override def paymentFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        var paymentPath = "/ge/dragon/warehouse/sdk/ifrs_recharge"
        var paymentPathList = DataUtils.getMultiFiles(paymentPath, logDate, 1)

        def paymentFilter(arr: Array[String]): Boolean = {
            return (arr.length == 15 && arr(4).startsWith(logDate) && arr(0) == "KFTL")
        }

        def paymentGenerate(arr: Array[String]): Row = {
            
            Row(gameCode, arr(4), arr(1).toLowerCase(), arr(14), arr(5), arr(11), arr(12))
        }

        var sf = Constants.PAYMENT_FIELD
        var paymentMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ID, sf.TRANS_ID, sf.SID, sf.GROSS_AMT, sf.NET_AMT)
