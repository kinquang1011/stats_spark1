...
object TsFormatter  extends IngameFormatter("ts", "ts") {
    ...
    override def loginLogoutFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        def loginFilter(arr: Array[String]): Boolean = {
            return (arr(1).startsWith(logDate))
        }
        
        def logoutFilter(arr: Array[String]): Boolean = {
            return (arr(2).startsWith(logDate))
        }

        def loginGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(1), "login", arr(0), arr(2), arr(3), "0")
        }
        
        def logoutGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(2), "logout", arr(0), arr(4), arr(5), arr(3))
        }

        var sf = Constants.LOGIN_LOGOUT_FIELD
        var loginMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.IP, sf.SID, sf.ONLINE_TIME)
        var logoutMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.IP, sf.SID, sf.ONLINE_TIME)
    ...
    override def paymentFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        def paymentGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(5), arr(1), arr(10), arr(6), arr(6), arr(4), arr(0))
        }

        var sf = Constants.PAYMENT_FIELD
        var paymentMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ID, sf.CHANNEL, sf.GROSS_AMT, sf.NET_AMT, sf.SID, sf.TRANS_ID)
