...
object NarutoFormatter extends IngameFormatter("naruto", "naruto") {
    ...
    override def loginLogoutFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        def loginFilter(arr: Array[String]): Boolean = {
            return (arr.length == 5 && arr(0).startsWith(logDate))
        }
        ...
        def loginGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(0), "login", arr(4), arr(1), arr(3))
        }
        
        def logoutGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(0), "logout", arr(4), arr(1), arr(3))
        }

        var sf = Constants.LOGIN_LOGOUT_FIELD
        var loginMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.RID, sf.SID)
        var logoutMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ACTION, sf.ID, sf.RID, sf.SID)
    ...
    override def paymentFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        def paymentGenerate(arr: Array[String]): Row = {
            Row(gameCode, arr(0), arr(1), (arr(4).toLong * 100).toString(), (arr(4).toLong * 100).toString(), arr(6), arr(5))
        }

        var sf = Constants.PAYMENT_FIELD
        var paymentMapping: Array[String] = Array(sf.GAME_CODE, sf.LOG_DATE, sf.ID, sf.GROSS_AMT, sf.NET_AMT, sf.SID, sf.TRANS_ID)
