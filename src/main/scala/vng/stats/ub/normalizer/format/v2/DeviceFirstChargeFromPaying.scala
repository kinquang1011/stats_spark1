 ...
 */
class DeviceFirstChargeFromPaying(_gameCode:String, _logDate:String, _config: Array[FormatterConfig], _isSdkLog: Boolean = true)
    extends DeviceFirstChargeFormatter (_gameCode, _logDate, _config) {
    monitorCode = "dfcfp"
    var totalAccPath = ""
    ...
    totalAccBeforePath = Common.getOuputParquetPath(gameCode, Constants.PARQUET_2.TOTAL_DEVICE_PAID_OUTPUT_FOLDER, dateBeforeOneDay, _isSdkLog)
    totalAccPath = Common.getOuputParquetPath(gameCode, Constants.PARQUET_2.TOTAL_DEVICE_PAID_OUTPUT_FOLDER, logDate, _isSdkLog)
    ...
    def appendTotalAcc(newAcc: DataFrame, path: String, sqlContext: SQLContext): Unit = {
        ...
        if (full1 == null) {
            Common.logger("Both total paid device and payment are null")
        } else {
            full1.cache
            full1.count
            Common.logger("Total paid device will be write in: " + totalAccPath)
    ...
    def firstChargeReset(data: DataFrame): Boolean = {
        ...
        var a_c: Long = paymentData.select("did").distinct.count()
        var n_c = data.select("did").distinct.count()

        if (a_c == n_c) {
            var dateBeforeOneDay = DateTimeUtils.getDateDifferent(-1, logDate, Constants.TIMING, Constants.A1)
            var firstBeforePath = Common.getOuputParquetPath(gameCode, Constants.PARQUET_2.DEVICE_FIRST_CHARGE_OUTPUT_FOLDER, dateBeforeOneDay, _isSdkLog)
            ...
            if(b.isSuccess && t1.count() != 0){
                ...
                Common.logger(mess)
                insertMonitorLog(Constants.ERROR_CODE.DEVICE_FIRST_CHARGE_RESET, mess, Constants.ERROR_CODE.WARNING)
            }
        }
        isReset
    }
