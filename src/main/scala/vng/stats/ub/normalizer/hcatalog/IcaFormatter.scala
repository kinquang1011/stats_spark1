...
import vng.stats.ub.normalizer.hcatalog.DttkFormatter._
...
object IcaFormatter extends IngameFormatter ("icamfbs2", "ica") {
    ...
    override def paymentFormatter(logDate: String, sc: SparkContext): Unit = {
        ...
        var rechargeQuery = new HiveQueryConfig {
            ...
            select("payment_gateway", sf.CHANNEL)
            select("level", sf.LEVEL)
        ...
        var fields: Map[String, Array[String]] = Map("A" -> Array(sf.GAME_CODE, sf.LOG_DATE, sf.ID,sf.SID,
            sf.GROSS_AMT, sf.NET_AMT, sf.LEVEL, sf.TRANS_ID, sf.IP, sf.PAY_CHANNEL, sf.CHANNEL),
            "B" -> Array(sf.OS))
        var formatOS: FormatterConfig = new FormatterConfig(null, null, Array(sf.OS), activityPath, Map("type" -> "left join", "fields" -> fields, "where" -> where, "on" -> on))
