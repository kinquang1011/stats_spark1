...
object KpiServerAccountRegister extends BaseReport {

    var accregisterPath = ""
    
    override def readExtraParams(): Unit = {
        
        accregisterPath = parameters(Constants.Parameters.SERVER_ACC_REGISTER_PATH)
    }
    ...
    def excute(sqlContext: SQLContext): DataFrame = {
        ...
        resultDF.collect().foreach { row =>
            ...
            var id = DataUtils.getKpiId(calcId, Constants.Kpi.SERVER_NEW_ACCOUNT_PLAYING, timing)
